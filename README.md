...
* **📝 Manual Entry** — add jobs by hand with optional JD paste for AI parsing; automatic **duplicate detection** (matching URL or company+role)
* **🗑 Remove Application** — interactive removal of entries from the sheet
...
python -m gmail
```
This will open a browser window asking you to sign in to Google and grant read-only Gmail access. If no browser is available (e.g. over SSH), a URL is printed in the terminal — copy-paste it into a browser manually. After authorizing, a `gmail_token.json` file is created automatically in the project root. You only need to do this once.

> **Troubleshooting**: If authorization fails, delete `gmail_token.json` and re-run `python -m gmail`. Make sure your Gmail address is added as a **Test user** in the OAuth consent screen. If you see a `UnicodeEncodeError`, update to Python 3.12+ or set `PYTHONIOENCODING=utf-8` to avoid emoji encoding issues in older Windows terminals.
...
All operations are run via `agent.py` from the project root.

| Command | Description | Example |
|---------|-------------|---------|
| `add` | Interactive job entry with duplicate detection (by URL or company+role) | `python agent.py add` |
| `list` | View all tracked applications; filter by optional `--status` | `python agent.py list --status Interviewing` |
| `update` | Interactively pick a job and change status / notes / follow-up dates | `python agent.py update` |
| `remove` | Interactively pick and permanently delete a job from the sheet | `python agent.py remove` |
| `followup` | Show applications with overdue or due-soon follow-ups | `python agent.py followup` |
| `check-emails` | Scan Gmail inbox for unread job-related emails, classify with AI, and auto-update the sheet | `python agent.py check-emails` |

#### Additional Scripts

```bash
# First-time Gmail OAuth authorization
python -m gmail                        # opens browser or prints a URL for headless use
...
Two entry points serve the system. `agent.py` provides the CLI for manual operations (add, list, update, remove, followup, check-emails). `scheduler.py` runs unattended via `--once` (triggered by Task Scheduler, launchd, or systemd) or as a persistent daemon. Both fetch unread job emails through `gmail.py`, classify them with `ai_parser.py` (Gemini), and sync to Google Sheets via `sheets.py` — matching by email thread ID or company+role. `sheets.py` auto-creates the "Applications" worksheet if it does not exist and provides duplicate detection for manual entries. All configuration flows through `config.py` from environment variables with sensible defaults.
...
* **Gmail auth fails** — delete `gmail_token.json` and re-run `python -m gmail`; if no browser opens, the URL is printed in the terminal for manual access
